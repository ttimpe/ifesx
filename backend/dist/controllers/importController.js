...
importRouter.post('/gtfs', upload.single('file'), async (req, res) => {
    try {
        if (!req.file) {
...
importRouter.post('/stopInfo', upload.single('file'), async (req, res) => {
    try {
        if (!req.file) {
            return res.status(400).json({ message: 'No file uploaded' });
        }
...
async function parseStopInfo(content) {
    return new Promise((resolve, reject) => {
        const stopInfosToCreate = [];
        const stream = (0, parse_1.parse)({ headers: true })
            .on('data', (row) => {
            // Add each row to the array to be bulk created
            if (row['ID'] != '') {
            ...
            try {
                ...
                stopInfosToCreate.forEach(async (stopInfo) => {
                    let stopsWithId = await Stop_1.Stop.findAll({
                        where: {
                            id: {
                                [sequelize_1.Op.like]: stopInfo.stop_id + '%'
                            }
                        },
                    ...
                    stopsWithId.forEach(async (stopWithId) => {
                        if (!stopWithId.stopInformation) {
                            stopWithId.stopInformation = new StopInformation_1.StopInformation();
                        }
