...
class KursblattController {
    ...
    async getVariantStops(liNr, varId) {
        ...
        return verlauf.map((v) => {
            ...
            return ({
                ...
                ORT_REF_ORT_NAME: (_d = v.ort) === null || _d === void 0 ? void 0 : _d.ORT_REF_ORT_NAME,
    ...
    async generatePdf(req, res) {
        ...
        try {
            ...
            let basisVersionText = (basisVersion === null || basisVersion === void 0 ? void 0 : basisVersion.BASIS_VERSION_TEXT) || `Version ${umlauf.BASIS_VERSION}`;
            if (basisVersion === null || basisVersion === void 0 ? void 0 : basisVersion.GUELTIG_AB) {
            ...
            for (const key of variantKeys) {
                ...
                const lid = await RecLid_1.RecLid.findOne({
                    where: {
                        LI_NR: liNr,
                        STR_LI_VAR: varStr,
            ...
            while (taskQueue.length > 0) {
                ...
                const renderChunk = async (trips) => {
                    ...
                    tableBody.push([
                        ...
                        ...displayStops.map((s) => ({ text: s.ORT_REF_ORT_KUERZEL || s.ORT_REF_ORT_NAME || s.ORT_NAME, style: 'tableCell' })),
                        { text: 'Route', style: 'tableCell', bold: true },
                        { text: 'WZ', style: 'wzCell', bold: true }
                    ]);
                    ...
                    for (const trip of trips) {
                        ...
                        for (let i = 0; i < tripStops.length; i++) {
                            ...
                            if (i < tripStops.length - 1) {
                                ...
                                currentSeconds += ((_a = fzt === null || fzt === void 0 ? void 0 : fzt.SEL_FZT) !== null && _a !== void 0 ? _a : 0);
                            }
                        }
                        ...
                        for (const tStop of tripTimeSequence) {
                            ...
                            let bestMatch = -1;
                            if (direction === 1) {
                                bestMatch = (_b = matches.find(m => m > colIdx)) !== null && _b !== void 0 ? _b : -1;
                                if (bestMatch === -1)
                                    bestMatch = matches[0];
                            }
                            else {
                                bestMatch = (_c = matches.reverse().find(m => m < colIdx)) !== null && _c !== void 0 ? _c : -1;
                                if (bestMatch === -1)
                                    bestMatch = matches[0];
                            }
                            if (bestMatch !== -1) {
                        ...
                        if (lastTripEndSeconds !== -1) {
                            ...
                            if (diff >= 0 && diff < 120) {
                                ...
                                if (tripTimeSequence[0].refId === firstColRef)
                                    wzA = waitStr;
                                else if (tripTimeSequence[0].refId === lastColRef)
                                    wzE = waitStr;
                                else
                                    wzA = waitStr;
                            }
                        ...
                        tableBody.push([
                            ...
                            ...rowData.map(d => ({ text: d ? d.text : '-', style: 'tableCell', decoration: d === null || d === void 0 ? void 0 : d.decoration })),
                            { text: direction !== 1 ? routeLabel : '', style: 'tableCell', bold: true },
                    ...
                    content.push({
                        columns: [{ width: '*', text: '' }, {
                                ...
                                layout: { fillColor: function (rowIndex) { if (rowIndex <= 1)
                                        return null; return (rowIndex % 2 === 0) ? '#e6e6e6' : null; } }
                            }, { width: '*', text: '' }]
                    });
                };
                ...
                const pageVars = new Map(); // Key: "Line-Var", Value: LineNr
                for (const t of pageTasks) {
                    if (t.type === 'trip' && ((_a = t.data) === null || _a === void 0 ? void 0 : _a.STR_LI_VAR)) {
                ...
                for (const [key, vLineNr] of pageVars) {
                    ...
                    if (lid && lid.LIDNAME)
                        ...
                        if (stops.length > 0)
                            desc += `: ${stops[0].ORT_NAME} - ${stops[stops.length - 1].ORT_NAME}`;
                    }
