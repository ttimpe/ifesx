...
class RecFrtController {
    constructor() {
        ...
        this.getAll = async (req, res) => {
            try {
                const basisVersion = req.query.basisVersion ? parseInt(req.query.basisVersion) : undefined;
                const where = {};
                if (basisVersion)
        ...
        this.getByUmlauf = async (req, res) => {
            try {
        ...
        this.getOrphanTrips = async (req, res) => {
            try {
                const basisVersion = parseInt(req.query.basisVersion);
                if (!basisVersion) {
                ...
                if (req.query.liNr)
                    where.LI_NR = parseInt(req.query.liNr);
                if (req.query.tagesartNr)
                    where.TAGESART_NR = parseInt(req.query.tagesartNr);
                const trips = await RecFrt_1.RecFrt.findAll({
                    where,
                    order: [['FRT_START', 'ASC']]
                });
        ...
        this.getNextFrtFid = async (req, res) => {
            try {
    ...
    async enrichTrips(trips, basisVersion) {
        if (!trips.length)
            return [];
        // 1. Extract Unique Line/Variant keys and Areas (Bereich)
        const variantKeys = new Set();
        const bereichNrs = new Set();
        trips.forEach(t => {
            if (t.LI_NR && t.STR_LI_VAR) {
                variantKeys.add(`${t.LI_NR}|${t.STR_LI_VAR}`);
            }
            if (t.BEREICH_NR)
                bereichNrs.add(t.BEREICH_NR);
        });
        if (bereichNrs.size === 0)
            bereichNrs.add(1); // Default
        ...
        const lids = await RecLid_1.RecLid.findAll({
            where: {
                BASIS_VERSION: basisVersion,
                [sequelize_1.Op.or]: orCondition
            }
        });
        // 3. Fetch Stops for Logic (End)
        const allVariantStops = await LidVerlauf_1.LidVerlauf.findAll({
        ...
        const sels = await RecSel_1.RecSel.findAll({
        ...
        const variantDurationMap = new Map(); // KEY: LI|VAR|BEREICH
        ...
        const orts = await RecOrt_1.RecOrt.findAll({
            where: {
                BASIS_VERSION: basisVersion,
