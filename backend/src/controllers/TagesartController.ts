...
export class TagesartController {
    ...
    async mergeTagesart(req: Request, res: Response) {
        ...
        const transaction = await RecFrt.sequelize!.transaction();

        try {
            // 1. Fetch Source Trips
            const sourceTrips = await RecFrt.findAll({
                where: {
            ...
            const targetTrips = await RecFrt.findAll({
                where: {
                    BASIS_VERSION: basisVersion,
            ...
            if (tripsToMove.length > 0) {
                await RecFrt.update({ TAGESART_NR: targetId }, {
                    where: {
                        BASIS_VERSION: basisVersion,
                        FRT_FID: tripsToMove
                    },
            ...
            await Betriebstag.update({ TAGESART_NR: targetId }, {
                where: {
                    BASIS_VERSION: basisVersion,
                    TAGESART_NR: sourceId
                },
            ...
            if (deleteSource) {
                ...
                // We will delete it.
                await Tagesart.destroy({ where: { TAGESART_NR: sourceId }, transaction });
            }
