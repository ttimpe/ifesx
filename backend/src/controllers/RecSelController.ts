...
export class RecSelController {
    ...
    public getByCompositeKey = async (req: Request, res: Response) => {
        try {
            const ortNr = parseInt(req.params.ortNr);
            const selZiel = parseInt(req.params.selZiel);
            const basisVersion = req.query.basisVersion ? Number(req.query.basisVersion) : 1;

            const sel = await RecSel.findOne({
    ...
    public updateByCompositeKey = async (req: Request, res: Response) => {
        try {
            const ortNr = parseInt(req.params.ortNr);
            const selZiel = parseInt(req.params.selZiel);
            ...
            const [updated] = await RecSel.update(req.body, {
                where: { ORT_NR: ortNr, SEL_ZIEL: selZiel, BASIS_VERSION: basisVersion }
            });
    ...
    public deleteByCompositeKey = async (req: Request, res: Response) => {
        try {
            const ortNr = parseInt(req.params.ortNr);
            const selZiel = parseInt(req.params.selZiel);
            const basisVersion = req.query.basisVersion ? Number(req.query.basisVersion) : 1;

            const deleted = await RecSel.destroy({
                where: { ORT_NR: ortNr, SEL_ZIEL: selZiel, BASIS_VERSION: basisVersion }
            });
    ...
    public create = async (req: Request, res: Response) => {
        try {
            ...
            const newItem = await RecSel.create(data);
            res.json(newItem);
        } catch (e) {
            res.status(500).json({ error: e });
        }
    ...
    public getFztByBereich = async (req: Request, res: Response) => {
        try {
            const bereichNr = parseInt(req.params.bereichNr);
            const basisVersion = req.query.basisVersion ? Number(req.query.basisVersion) : 1;

            const list = await RecSelFztFeld.findAll({
                where: {
    ...
    public updateFzt = async (req: Request, res: Response) => {
        try {
            ...
            // Upsert
            const [item, created] = await RecSelFztFeld.findOrCreate({
                where: {
                    BASIS_VERSION,
                    BEREICH_NR,
                    FGR_NR,
